...
8. **GitHub / `gh` (optional):** not in the Docker image — use **`gh` on an OpenClaw node** (e.g. Mac). See **[AUTOMATION.md](./AUTOMATION.md)**.
...
**Variables must appear in `docker-compose.prod.yml` under `environment:`** for each service. Putting a name only in `.env` is not enough: Compose uses `.env` for **substitution** of `${VAR:-}` in the YAML, but the container only receives vars that are **declared** in `environment:` (unless you add `env_file:`).
